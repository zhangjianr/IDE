...
# Try (in order): PHPSTORM_JDK, ../jre, JDK_HOME, JAVA_HOME, "java" in PATH.
# ---------------------------------------------------------------------
if [ -n "$PHPSTORM_JDK" -a -x "$PHPSTORM_JDK/bin/java" ]; then
  JDK="$PHPSTORM_JDK"
elif [ -s "$HOME/.PhpStorm2016.1/config/phpstorm.jdk" ]; then
  JDK=`$CAT $HOME/.PhpStorm2016.1/config/phpstorm.jdk`
  if [ ! -d $JDK ]; then
    JDK=$IDE_HOME/$JDK
  fi
...
elif [ -r "$HOME/.PhpStorm2016.1/phpstorm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.PhpStorm2016.1/phpstorm$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=PhpStorm2016.1"
  fi
fi
...
RESTARTER="$HOME/.PhpStorm2016.1/system/restart/restarter.sh"
if [ -x "$RESTARTER" ]; then
  "$RESTARTER"
  "$RM" -f "$RESTARTER"
fi
